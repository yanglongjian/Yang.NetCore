...
namespace Yang.Application.Admin
{
    ...
    public class RoleController : IDynamicApiController
    {
        ...
        /// <returns></returns>
        [ModuleInfo("读取")]
        public virtual async Task<object> Read(PageRequest request)
        {
            ...
            var rows = await DbContext.Instance.Queryable<Role>().Where(predicate)
        ...
        /// <returns></returns>
        [ModuleInfo("新增")]
        public virtual async Task<int> Create(Role entity)
        {
        ...
        [HttpPost]
        [ModuleInfo("更新")]
        public virtual async Task<int> Update(Role entity)
        {
        ...
        {
            return await DbContext.Instance.Deleteable<Role>().Where(r => ids.Contains(r.Id)).ExecuteCommandAsync();
        }
        ...
        //[HttpGet]
        //[ModuleInfo("读取角色权限")]
        //public virtual async Task<object> ReadRoleModules(int roleId)
        //{
        //    var checkedKeys = await DbContext.Instance.Queryable<ModuleRole>().Where(r => r.RoleId == roleId).Select(m => m.ModuleId).ToListAsync();
        //    var list = await DbContext.Instance.Queryable<Module>().ToListAsync();
