...
namespace Yang.Core
{
    ...
    public class ValidateCoder
    {
        ...
        /// 获取指定字符串的验证码图片
        /// </summary>
        private Bitmap CreateImage(string code, ValidateCodeType codeType)
        {
            ...
            for (int i = 0; i < code.Length; i++)
            {
                ...
                Font font = new(fontName, FontSize, FontStyle.Bold);
                if (RandomItalic)
                {
        ...
        /// 获取指定长度的验证码字符串
        /// </summary>
        public static string GetCode(int length, ValidateCodeType codeType = ValidateCodeType.NumberAndLetter)
        {
        ...
        /// <returns></returns>
        public static bool CheckVerifyCode(string constantKey, string id, string code, bool removeIfSuccess = true)
        {
            var _cache = App.GetService<IMemoryCache>();
            string key = $"{constantKey}_{id}";
            bool flag = code.Equals(_cache.Get<string>(key), StringComparison.OrdinalIgnoreCase);
            if (flag && removeIfSuccess)
            {
                _cache.Remove(key);
            }
            return flag;
